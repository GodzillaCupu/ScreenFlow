...
/// Archived scripts ("The Vault"). Refreshes whenever a script is saved
/// (archive/unarchive both go through save()).
final archivedScriptsProvider = FutureProvider<List<Script>>((ref) async {
  ref.watch(recentScriptsProvider); // invalidate on any script change
  final all =
      await ref.watch(scriptRepositoryProvider).getAll(includeArchived: true);
